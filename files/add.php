...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') {
        ...
        // Kiểm tra trùng tên
        $check = $pdo->prepare("SELECT id FROM lop_hoc WHERE name = ?");
        $check->execute([$name]);
        if ($check->fetch()) {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO lop_hoc (name, status) VALUES (?, ?)");
...
<head>
    ...
    <style>
        ...
        .sidebar {
            ...
            position: fixed; top: 0; left: 0; z-index: 100; padding-top: 1rem;
        }
        .sidebar-brand {
        ...
        .form-card {
            background: #fff; border-radius: 12px; max-width: 560px;
        ...
        .form-card-header i { font-size: 1.2rem; color: #2563eb; }
